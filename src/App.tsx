...
function Workspace() {
  ...
  const [remoteChannels, setRemoteChannels] = useState<{ id: string; name: string; isPrivate: boolean }[]>([]);
  ...
  const saveTask = async (task: Task, patch: Pick<Task, "title" | "due" | "value" | "priority" | "status" | "taskType">) => {
  ...
  const createTask = async (form: HTMLFormElement) => {
    ...
    const task: Task = { id: Date.now(), title: String(data.get("title")), company: String(data.get("company")) as Task["company"], category: String(data.get("category")), due: String(data.get("due")), owner: workspaceUsers.find((user) => user.id === responsibleId)?.name ?? String(data.get("owner") ?? "Não atribuído"), value: Number(data.get("value")), priority: "Média", status: "Pendente", listId: String(data.get("listId")), taskType: String(data.get("taskType")) as Task["taskType"] };
    if (usesRemoteTasks) {
      try {
