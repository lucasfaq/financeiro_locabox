...
function Workspace() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <div className="nav-group"><p>GERENCIAR</p>{[{ label: "Empresas", icon: Building2 }, { label: "Equipe", icon: Users }, { label: "Configurações", icon: Settings }].map(({ label, icon: Icon }) => <button key={label} className="nav-link" onClick={() => setNotice(`${label} será habilitado na próxima etapa.`)}><Icon size={18} />{label}</button>)}</div>
    ...
    <section className="workspace">
      ...
      <div hidden={section === "Templates" || section === "Caixa de Entrada" || section === "Documentos"}>
