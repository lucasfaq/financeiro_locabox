  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {!channelOpen && <section className="views-studio"><div className="views-heading"><div><p className="eyebrow">DEPARTAMENTO / FINANCEIRO</p><h2>Visões de tarefas</h2></div><div className="view-tabs">{(["Lista", "Quadro", "Calendário", "Gantt"] as const).map((view) => <button key={view} className={taskView === view ? "selected" : ""} onClick={() => setTaskView(view)}>{view}</button>)}{savedTaskViews.map((view) => <button key={view.id} title={`Aplicar ${view.name}`} onClick={() => applySavedTaskView(view)}>{view.name}</button>)}<button className="new-view" onClick={createSavedTaskView}>+ Nova visão</button></div></div>
        ...
        {taskView === "Gantt" && <div className="gantt-view"><div className="calendar-title gantt-controls"><button className="secondary-button" onClick={() => setCalendarCursor((current) => new Date(current.getFullYear(), current.getMonth() - 1, 1))}>‹</button><strong>{calendarCursor.toLocaleDateString("pt-BR", { month: "long", year: "numeric" })}</strong><button className="secondary-button" onClick={() => setCalendarCursor((current) => new Date(current.getFullYear(), current.getMonth() + 1, 1))}>›</button></div><div className="gantt-scale">{ganttScale(calendarCursor).map((label) => <span key={label}>{label}</span>)}</div>{visibleTasks.map((task) => <div className="gantt-row" key={task.id}><strong>{task.title}</strong><div><i style={ganttStyle(task, calendarCursor)}>{task.status}</i></div></div>)}</div>}
      </section>}
      </div>
    </section>
