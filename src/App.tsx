...
function Workspace() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {section === "Atividades" && !channelOpen && <section className="views-studio task-workspace-view"><div className="views-heading"><div><p className="eyebrow">{selectedList ? `${selectedList.department.toUpperCase()} / ${selectedList.folder.toUpperCase()}` : "ATIVIDADES"}</p><h2>{selectedList?.name ?? "Atividades"}</h2></div><div className="view-tabs">{(["Lista", "Quadro", "Calendário", "Gantt"] as const).map((view) => <button key={view} className={taskView === view ? "selected" : ""} onClick={() => setTaskView(view)}>{view}</button>)}{savedTaskViews.map((view) => <button key={view.id} title={`Aplicar ${view.name}`} onClick={() => applySavedTaskView(view)}>{view.name}</button>)}<button className="new-view" onClick={createSavedTaskView}>+ Nova visão</button></div></div>
