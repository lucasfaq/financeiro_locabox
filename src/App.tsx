...
function Workspace() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <div className="nav-group"><p>GERENCIAR</p>{[{ label: "Empresas", icon: Building2 }, { label: "Equipe", icon: Users }, { label: "Configurações", icon: Settings }].map(({ label, icon: Icon }) => <button key={label} className={section === label ? "nav-link active" : "nav-link"} onClick={() => { if (label === "Equipe") { setSection(label); setChannelOpen(false); } else setNotice(`${label} será habilitado na próxima etapa.`); }}><Icon size={18} />{label}</button>)}</div>
    ...
    <section className="workspace">
      ...
      <div hidden={section === "Templates" || section === "Caixa de Entrada" || section === "Documentos" || section === "Equipe"}>
