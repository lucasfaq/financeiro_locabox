...
import { addChannelMember, archiveFinancialTask, clearReadNotifications, createChannelMessage, createDirectMessage, createFinancialTask, createWorkspaceChannel, createWorkspaceDepartment, createWorkspaceDocument, createWorkspaceFolder, createWorkspaceList, createWorkspaceTemplate, decideFinancialApproval, deleteWorkspaceDocument, deleteWorkspaceTemplate, loadActiveWorkspaceUsers, loadArchivedFinancialTasks, loadChannelMembers, loadChannelMessages, loadFinancialTasks, loadNotifications, loadWorkspaceChannels, loadWorkspaceDocuments, loadWorkspaceHierarchy, loadWorkspaceTemplates, markAllNotificationsRead, markNotificationRead, removeChannelMember, snoozeNotification, updateFinancialTask, updateFinancialTaskStatus, updateWorkspaceDocument, updateWorkspaceTemplate, type RemoteChannelMember, type RemoteWorkspaceUser } from "./workspaceRepository";
...
function Workspace() {
  ...
  const [taskGroupBy, setTaskGroupBy] = usePersistentState<"Nenhum" | "Status" | "Prioridade" | "Empresa">("itp-financeiro-task-group-by", "Nenhum");
  ...
  const taskGroups = useMemo(() => visibleTasks.reduce<Record<string, Task[]>>((groups, task) => {
    const group = taskGroupBy === "Status" ? task.status : taskGroupBy === "Prioridade" ? task.priority : taskGroupBy === "Empresa" ? task.company : "Atividades";
    (groups[group] ??= []).push(task);
    return groups;
  }, {}), [visibleTasks, taskGroupBy]);
  const activeFilterCount = Number(filter !== "Todos") + Number(companyFilter !== "Todas") + Number(priorityFilter !== "Todas");
  ...
  const deleteSavedTaskView = (view: SavedTaskView) => {
    if (!window.confirm(`Excluir a visualização “${view.name}”?`)) return;
    setSavedTaskViews((current) => current.filter((item) => item.id !== view.id));
    setNotice(`Visualização “${view.name}” excluída.`);
  };
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {(["Visão geral", "Atividades"] as string[]).includes(section) && !channelOpen && <div className="task-command-bar"><div className="task-command-actions"><label>Agrupar<select value={taskGroupBy} onChange={(event) => setTaskGroupBy(event.target.value as typeof taskGroupBy)}><option>Nenhum</option><option>Status</option><option>Prioridade</option><option>Empresa</option></select></label><label>Ordenar<select value={taskOrder} onChange={(event) => setTaskOrder(event.target.value as typeof taskOrder)}><option value="recentes">Recentes</option><option value="vencimento">Vencimento</option><option value="valor">Maior valor</option></select></label><button className={filterPanelOpen || activeFilterCount ? "command-filter active" : "command-filter"} onClick={() => setFilterPanelOpen((current) => !current)}>Filtros{activeFilterCount > 0 && <b>{activeFilterCount}</b>}</button></div><label className="command-search"><Search size={15}/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar tarefas" /></label>{filterPanelOpen && <section className="filter-panel"><header><strong>Filtros</strong><button className="text-button" onClick={() => { setFilter("Todos"); setCompanyFilter("Todas"); setPriorityFilter("Todas"); }}>Limpar</button></header><div><label>Status<select value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)}><option>Todos</option><option>Pendente</option><option>Em aprovação</option><option>Aprovado</option><option>Executado</option></select></label><label>Empresa<select value={companyFilter} onChange={(event) => setCompanyFilter(event.target.value as typeof companyFilter)}><option>Todas</option><option>ITP</option><option>Locabox</option></select></label><label>Prioridade<select value={priorityFilter} onChange={(event) => setPriorityFilter(event.target.value as typeof priorityFilter)}><option>Todas</option><option>Alta</option><option>Média</option><option>Baixa</option></select></label></div></section>}</div>}
      {notice && <div className="notice"><Check size={17} />{notice}</div>}
      {section === "Templates" && <TemplatesLibrary templates={templates} onCreate={async (template) => { if (!usesRemoteTasks) { setTemplates((current) => [{ id: Date.now(), ...template }, ...current]); setNotice("Template criado para este espaço de trabalho."); return; } const saved = await createWorkspaceTemplate(template); setTemplates((current) => [saved, ...current]); setNotice("Template criado para a equipe."); }} onUse={(template) => { setTemplateForTask(template); setShowModal(true); setNotice(`Template “${template.name}” aplicado à nova tarefa.`); }} onUpdate={async (id, patch) => { if (usesRemoteTasks && typeof id === "string") await updateWorkspaceTemplate(id, patch); setTemplates((current) => current.map((template) => template.id === id ? { id, ...patch } : template)); }} onDelete={async (id) => { if (usesRemoteTasks && typeof id === "string") await deleteWorkspaceTemplate(id); setTemplates((current) => current.filter((template) => template.id !== id)); }} />}
      ...
      {section === "Documentos" && <DocumentsWorkspace documents={documents} savingIds={savingDocumentIds} onCreate={async () => { if (!usesRemoteTasks) { const document = { id: Date.now(), title: "Sem título", body: "", updated: "agora" }; documentsRef.current = [document, ...documentsRef.current]; setDocuments((current) => [document, ...current]); return document; } const document = await createWorkspaceDocument(); documentsRef.current = [document, ...documentsRef.current]; setDocuments((current) => [document, ...current]); return document; }} onUpdate={saveDocumentDraft} onDelete={async (id) => { if (usesRemoteTasks && typeof id === "string") await deleteWorkspaceDocument(id); window.clearTimeout(documentSaveTimers.current[String(id)]); delete documentSaveTimers.current[String(id)]; setSavingDocumentIds((ids) => ids.filter((item) => item !== id)); const remaining = documentsRef.current.filter((document) => document.id !== id); documentsRef.current = remaining; setDocuments(remaining); }} />}
      ...
      <div className={section === "Visão geral" ? "summary-grid" : "summary-grid overview-hidden"}>
      ...
      {!channelOpen && <div className={section === "Visão geral" ? "content-grid" : "content-grid overview-hidden"}>
      ...
      {(section === "Visão geral" || channelOpen) && <section className={channelOpen ? "channel-workspace" : "collaboration-strip"}>
      ...
      {section === "Atividades" && !channelOpen && <section className="views-studio task-workspace-view"><div className="views-heading"><div><p className="eyebrow">{selectedList ? `${selectedList.department.toUpperCase()} / ${selectedList.folder.toUpperCase()}` : "ATIVIDADES"}</p><h2>{selectedList?.name ?? "Atividades"}</h2></div><div className="view-tabs">{(["Lista", "Quadro", "Calendário", "Gantt"] as const).map((view) => <button key={view} className={taskView === view ? "selected" : ""} onClick={() => setTaskView(view)}>{view}</button>)}{savedTaskViews.map((view) => <span className="saved-view" key={view.id}><button title={`Aplicar ${view.name}`} onClick={() => applySavedTaskView(view)}>{view.name}</button><button className="saved-view-delete" title={`Excluir ${view.name}`} aria-label={`Excluir visualização ${view.name}`} onClick={() => deleteSavedTaskView(view)}>×</button></span>)}<button className="new-view" onClick={createSavedTaskView}>+ Nova visão</button></div></div>
        {taskView === "Lista" && <div className="task-groups">{Object.entries(taskGroups).map(([group, groupedTasks]) => <section key={group}><h3>{taskGroupBy === "Nenhum" ? "Atividades" : group}<small>{groupedTasks.length}</small></h3><div className="compact-list">{groupedTasks.map((task) => <button className="task-list-entry" key={task.id} onClick={() => setDetailTask(task)}><span className={`priority ${task.priority.toLowerCase()}`}/><strong>{task.title}</strong><small>{task.owner} · {task.due}</small><span className={`status ${task.status.toLowerCase().replace(" ", "-")}`}>{task.status}</span></button>)}</div></section>)}</div>}
        {taskView === "Quadro" && <div className="kanban">{(["Pendente", "Em aprovação", "Aprovado", "Executado"] as Status[]).map((status) => <div className="kanban-column" key={status} onDragOver={(event) => event.preventDefault()} onDrop={() => void moveTaskToStatus(status)}><header><strong>{status}</strong><span>{visibleTasks.filter((task) => task.status === status).length}</span></header>{visibleTasks.filter((task) => task.status === status).map((task) => <article key={task.id} draggable onClick={() => setDetailTask(task)} onDragStart={() => setDraggedTaskId(task.id)} onDragEnd={() => setDraggedTaskId(null)}><span className={task.priority === "Alta" ? "tag red-tag" : "tag"}>{task.company}</span><strong>{task.title}</strong><p>{task.owner} · {task.due}</p></article>)}</div>)}</div>}
        ...
        {taskView === "Gantt" && <div className="gantt-view"><div className="calendar-title gantt-controls"><button className="secondary-button" onClick={() => setCalendarCursor((current) => new Date(current.getFullYear(), current.getMonth() - 1, 1))}>‹</button><strong>{calendarCursor.toLocaleDateString("pt-BR", { month: "long", year: "numeric" })}</strong><button className="secondary-button" onClick={() => setCalendarCursor((current) => new Date(current.getFullYear(), current.getMonth() + 1, 1))}>›</button></div><div className="gantt-scale">{ganttScale(calendarCursor).map((label) => <span key={label}>{label}</span>)}</div>{visibleTasks.map((task) => <button className="gantt-row" key={task.id} onClick={() => setDetailTask(task)}><strong>{task.title}</strong><div><i style={ganttStyle(task, calendarCursor)}>{task.status}</i></div></button>)}</div>}
      </section>}
      </div>
    </section>
    ...
    {detailTask && <TaskDetail task={detailTask} sidebarCollapsed={sidebarCollapsed} onClose={() => setDetailTask(null)} onArchive={() => void archiveTask(detailTask)} onSave={(patch) => saveTask(detailTask, patch)} />}
  </main>;
}
