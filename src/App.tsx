...
function Workspace() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <div className="task-filter-console"><label>Empresa<select value={companyFilter} onChange={(event) => setCompanyFilter(event.target.value as typeof companyFilter)}><option>Todas</option><option>ITP</option><option>Locabox</option></select></label><label>Prioridade<select value={priorityFilter} onChange={(event) => setPriorityFilter(event.target.value as typeof priorityFilter)}><option>Todas</option><option>Alta</option><option>Média</option><option>Baixa</option></select></label><label>Ordenar<select value={taskOrder} onChange={(event) => setTaskOrder(event.target.value as typeof taskOrder)}><option value="recentes">Mais recentes</option><option value="vencimento">Vencimento</option><option value="valor">Maior valor</option></select></label><label>Agrupar<select value={taskGroupBy} onChange={(event) => setTaskGroupBy(event.target.value as typeof taskGroupBy)}><option>Nenhum</option><option>Status</option><option>Prioridade</option><option>Empresa</option></select></label></div>
