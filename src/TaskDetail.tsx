...
export function TaskDetail({ task, onClose, onArchive, onSave }: { task: EditableTask; onClose: () => void; onArchive?: () => void; onSave?: (patch: Pick<EditableTask, "title" | "description" | "category" | "start" | "due" | "value" | "priority" | "status" | "taskType">) => Promise<void> }) {
  ...
  const removeSubtask = async (item: { id: string | number; title: string }) => {
    if (!window.confirm(`Excluir a subtarefa “${item.title}”?`)) return;
    if (persistent && typeof item.id === "string") { try { await deleteTaskSubtask(item.id); } catch { setMessage("Não foi possível excluir a subtarefa."); return; } }
