...
export function TaskDetail({ task, onClose, onArchive, onSave }: { task: EditableTask; onClose: () => void; onArchive?: () => void; onSave?: (patch: Pick<EditableTask, "title" | "description" | "start" | "due" | "value" | "priority" | "status" | "taskType">) => Promise<void> }) {
  ...
  const [edit, setEdit] = useState({ title: task.title, description: task.description ?? "", start: dateInputValue(task.start ?? ""), due: dateInputValue(task.due), value: String(task.value), priority: task.priority, status: task.status, taskType: task.taskType });
  ...
  useEffect(() => { setEdit({ title: task.title, description: task.description ?? "", start: dateInputValue(task.start ?? ""), due: dateInputValue(task.due), value: String(task.value), priority: task.priority, status: task.status, taskType: task.taskType }); setEditing(false); }, [task]);
