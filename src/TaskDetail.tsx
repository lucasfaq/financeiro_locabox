...
export type DetailTask = { id: string | number; title: string; company: string; category: string; start?: string; due: string; owner: string; value: number; status: "Pendente" | "Em aprovação" | "Aprovado" | "Executado" };
...
export function TaskDetail({ task, onClose, onArchive, onSave }: { task: EditableTask; onClose: () => void; onArchive?: () => void; onSave?: (patch: Pick<EditableTask, "title" | "start" | "due" | "value" | "priority" | "status" | "taskType">) => Promise<void> }) {
  ...
  const [edit, setEdit] = useState({ title: task.title, start: dateInputValue(task.start ?? ""), due: dateInputValue(task.due), value: String(task.value), priority: task.priority, status: task.status, taskType: task.taskType });
  ...
  useEffect(() => { setEdit({ title: task.title, start: dateInputValue(task.start ?? ""), due: dateInputValue(task.due), value: String(task.value), priority: task.priority, status: task.status, taskType: task.taskType }); setEditing(false); }, [task]);
